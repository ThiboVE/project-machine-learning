...
#         n_edge_features = 6
#         edge_feat_mat = np.zeros((n_atoms, n_atoms, n_edge_features))
...
#             # one-hots for bond types
#             is_single   = int(bond_type == Chem.rdchem.BondType.SINGLE)
#             is_double   = int(bond_type == Chem.rdchem.BondType.DOUBLE)
#             is_triple   = int(bond_type == Chem.rdchem.BondType.TRIPLE)
#             is_aromatic = int(bond_type == Chem.rdchem.BondType.AROMATIC)

#             is_conjug = int(bond.GetIsConjugated())
...
#             edge_feat_mat[i, j] = features
#             edge_feat_mat[j, i] = features

#         # pad if needed
#         dim_add = n_atoms - edge_feat_mat.shape[0]
#         edge_feat_mat = np.pad(
#             edge_feat_mat,
#             pad_width=((0, dim_add), (0, dim_add), (0, 0)),
#             mode="constant"
#         )

#         self.edge_feat_mat = edge_feat_mat
...
#         node_mat = torch.Tensor(mol.node_mat)
#         adj_mat = torch.Tensor(mol.adj_mat)
#         edge_mat = torch.Tensor(mol.edge_feat_mat)

#         # Get output
#         output = torch.Tensor([self.outputs[i]])

#         return (node_mat, adj_mat, edge_mat), output, smile
...
#     # Iterate over list and assign each component to the correct list
#     for i in range(len(dataset)):
#         (node_mat,adj_mat, edge_mat), output, smile = dataset[i]
#         node_mats.append(node_mat)
#         adj_mats.append(adj_mat)
...
#     node_mats_tensor = torch.cat(node_mats, dim=0)
#     adj_mats_tensor = torch.cat(adj_mats, dim=0)
#     edge_mats_tensor = torch.cat(edge_mats, dim=0)
#     outputs_tensor = torch.stack(outputs, dim=0)
    
#     # Return tensors
#     return (node_mats_tensor, adj_mats_tensor, edge_mats_tensor), outputs_tensor, smiles
...
#         n_edge_features = 6
#         self.edge_mlp = nn.Linear(node_in_len + n_edge_features, node_out_len)


#     def forward(self, node_mat, adj_mat, edge_mat):
#         B, N, F = node_mat.shape
#         _, _, _, E = edge_mat.shape

#         # expand node features to (B, N, N, F)
#         node_i = node_mat.unsqueeze(2).expand(B, N, N, F)
#         node_j = node_mat.unsqueeze(1).expand(B, N, N, F)

#         # concatenate node_j and edge_ij
#         combined = torch.cat([node_j, edge_mat], dim=-1)   # (B, N, N, F+E)

#         # apply transformation
#         messages = self.edge_mlp(combined)                 # (B, N, N, F_out)

#         # mask by adjacency
#         messages = messages * adj_mat.unsqueeze(-1)        # keep only neighbors

#         # sum over neighbors
#         node_fea = messages.sum(dim=2)
...
#         # Perform convolutions
#         for conv in self.conv_layers:
#             node_fea = conv(node_fea, adj_mat, edge_mat)
...
#         node_mat = node_mat.reshape(first_dim, max_atoms, node_vec_len)
#         adj_mat = adj_mat.reshape(first_dim, max_atoms, max_atoms)
#         edge_mat = edge_mat.reshape(first_dim, max_atoms, max_atoms, 6)
...
#         # Package inputs and outputs; check if GPU is enabled
#         if use_GPU:
#             nn_input = (node_mat.cuda(), adj_mat.cuda(), edge_mat.cuda())
#             nn_output = output_std.cuda()
#         else:
#             nn_input = (node_mat, adj_mat, edge_mat)
#             nn_output = output_std
