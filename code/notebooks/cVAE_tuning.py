...
def create_model(trial, fixed_params=None):
    """
...
def main():
    ...
    for fold, (outer_train_idx, outer_test_idx) in enumerate(outer_cv.split(dataset_indices, outer_binned_gaps)):
        ...
        # n_epochs = best_params['n_epochs']  # Now full tuned value
        ...
        # # Dummy trial for create_model (no suggests)
        # class DummyTrial:
        #     def suggest_float(self, name, low, high, log=False):
        #         return best_params.get(name, 0.01)
        #     def suggest_int(self, name, low, high):
        #         return best_params.get(name, 16)
        
        # dummy_trial = DummyTrial()
        # model, _, lr = create_model(dummy_trial)
        # optimizer = torch.optim.Adam(model.parameters(), lr=lr)
        ...
        # test_loss, test_acc, best_params
