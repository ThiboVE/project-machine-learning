...
The best set of hyperparameters is then used to generate new molecules in two ways: by randomly sampling latent vectors from the latent space and by perturbing latent vectors of dataset molecules, which are both then decoded conditioned on target HOMO-LUMO gaps. Generated molecules were evaluated by sampling for both experimental pathways 10 different molecules and for each molecule doing a 1000 generations, for which the average chemical validity and MAE are calculated per molecule, as described in \cref{sec:evalmetrics}.
...
Across the five outer folds, the selected hyperparameters were largely consistent, three out of the five fold ended on the same set of parameters, while fold 2 and 5 managed to get a different set (\cref{tab:cvae_params}). Between the five sets, the major differences lie in the size 


The final hyperparameter configurations for each outer fold are summarized in Table X.

Overall the nested cross-validation strategy provided a robust estimate of the model performance and reduced the risk of overfitting to a single validation split. However the limited amount of hyperparameter tuning that was able to be done suggests that further gains may be achievable with a more extensive optimization strategy.

The reconstruction accuracy of the model, using $\overline{CRA}$ (\cref{eq:CRA}), across the five outer folds are reported in \cite{tab:CRA}. The model achieved a mean reconstruction accuracy of 0.852, indicating it is capable of reconstructing specific molecules.
...
Beyond reconstruction, the more important metric of interest is the generative capability of the cVAE. The chemical validity of the generated molecules can be found in \cref{tab:} for both the random selected latent vectors and the perturbed latent vectors.
