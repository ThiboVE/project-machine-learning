...
\usepackage[table,xcdraw]{xcolor} % Voor kleurgebruik in tabellen
\usepackage{siunitx}
\usepackage{url}
\usepackage{pdfpages}
\usepackage{braket}
...
\begin{abstract}
  Accurate predictions of molecular electronic properties such as the HOMO–LUMO gap and inverse design of molecules based on such properties are critical for materials discovery and molecular engineering, but remain costly and time consuming for experimental methods and computationally expensive for quantum-chemical methods. While machine learning has gained increased relevance in such applications as a less expensive alternative, modern models still fail to reliably produce chemically viable results. In this study, we benchmark a LightGBM model against a GCN model for property prediction and a VAE-based model for molecular design. Our results show that the LightGBM and GCN model achieve comparable HOMO-LUMO gap prediction performance, while the VAE model shows good reconstruction capabilities, but limited chemical validity and property control for the generation of new molecules. A more extensive comparison between traditional and deep-learning approaches for HOMO–LUMO gap prediction could better identify the most effective architecture, while further refinements of the cVAE model are necessary to reliably generate molecules from target HOMO–LUMO gaps.
\end{abstract}
...
\subsection{LightGBM}\label{sec:lightgbm}
\noindent
...
\newline
\noindent
Beyond reconstruction performance, the primary metric of interest is the generative capability of the cVAE. Using the best-performing trained model (outer fold 3), the chemical validity of molecules generated from both randomly sampled latent vectors and perturbed latent vectors is evaluated (\cref{fig:validity} and \cref{tab:cvae_samples}). Across the ten randomly sampled target molecules, validities are obtained between $1.1\%$ and $6.6\%$ with an average of $4.5\%$ while for the perturbed samples, equally low validities are obtained between $1.8\%$ and $6.1\%$ with an average of $4.2\%$.
\newline
\newline
\noindent
For each target molecule, the mean absolute error (MAE) between the HOMO-LUMO gaps predicted by the GCN model and the corresponding target gaps used to condition the cVAE (\cref{section:cvae_method}) is computed and reported in \cref{fig:cvae_maes,tab:cvae_samples}. It is important to note that this MAE is calculated per molecule and depends on the number of valid generations, as only valid molecules contribute to the error calculation. Consequently, the validity indirectly influences the MAE. The perturbed and random latent sampling methods yield MAEs averaging on 0.739 eV and 0.763 eV respectively. For both methods, that is considerably larger than the errors reported for the standalone GCN model, meaning there is a significant difference between the target and generated HOMO-LUMO gap.
\newline
\newline
\noindent
...
\newline
\noindent
Although prior work has shown that deep-learning models do not consistently outperform non-deep models for molecular property prediction\parencite{NEURIPS2023_cc83e973}, our results show that the GCN attains comparable accuracy to that of the LightGBM model. While the GCN model learns chemically meaningful structural relationships through a graph, the LightGBM model uses descriptors of the molecule to learn mappings between these features and the target property. Rather than clearly favoring one approach over the other, our findings suggest that deep-learning and traditional models address the the prediction of HOMO-LUMO gap from different perspectives while achieving similar overall performance. A more comprehensive comparison across a wider range of models and datasets, similar to the analysis in \textcite{NEURIPS2023_cc83e973} but focused specifically on HOMO-LUMO gap prediction, would help clarify the relative advantages of traditional versus deep-learning approaches for this specific task.

\subsection{Molecular Generation with cVAE}

The cVAE model achieves a mean character-wise reconstruction accuracy of $\overline{CRA} = 0.852$ (\cref{tab:CRA}). While reconstruction accuracies for cVAEs on the exact same dataset are not available, \textcite{blaschke_2018} report 0.862 with teacher forcing (0.963 without) for a standard VAE, and \textcite{mollaysa_2024} achieved 0.961 with a Transformer VAE. These results show that our cVAE performs just below literature standard, however, our limited hyperparameter tuning suggests further improvements are possible.
\newline
\newline
\noindent
...
\newline
\noindent
Finally, the large mean absolute errors observed for the HOMO-LUMO gaps of generated molecules further indicate limited conditional control (\cref{tab:cvae_samples,fig:cvae_maes}). The errors are substantially larger than those reported in literature \parencite{bombarelli_2018}, highlighting that the conditional encoding is insufficient to reliably enforce target electronic properties, especially given low chemical validity.
...
The LightGBM model achieved the best predictive performance with a mean MAE of about 0.14 eV and an $R^2$ of 0.97, while the GCN reached a slightly higher MAE of roughly 0.16 eV but an equally strong $R^2$ of 0.97, indicating that both models reliably capture the HOMO–LUMO gap with comparable accuracy. The close agreement between LightGBM and GCN highlights that descriptor-based gradient boosting and graph-based deep learning offer distinct yet similarly effective predictions of the HOMO-LUMO gap. The cVAE demonstrated solid reconstruction performance (CRA = 0.85), but its generative results revealed low chemical validity and large errors in conditioned HOMO–LUMO gaps, pointing to limited control over property-targeted molecular generation. These findings suggest that while the cVAE learns coarse molecular patterns, its latent space does not yet encode sufficiently detailed structural and electronic information to support reliable conditional generation. Improving generative validity and conditional accuracy will likely require refined training strategies, richer molecular representations, and more expressive architectures, while predictive modeling could benefit from broader benchmarking across different datasets and models.
...
Portions of this report were improved using ChatGPT version 5 to refine writing style and enhance clarity. Furthermore, ChatGPT was also used to adapt existing code for the implementation of the machine learning models. All scientific decisions, experiments, analyses and interpretations were done by the authors themselves.
...
Mean & \multicolumn{1}{c|}{0.045} & 0.763 & \multicolumn{1}{c|}{0.042} & 0.739 \\ \hline
\end{tabular}
\end{table}
