...
\begin{abstract}
  \textcolor{purple}{In the abstract of your paper, briefly summarize your research in about 150 to 250 words. Briefly explain the problem statement, the techniques you used, and your general results. You can't go into deep detail here, of course, but you don't have to. Think of this as a kind of written ``elevator pitch'': explain your research to someone who has 1-2 minutes time to listen.}
  Accurate prediction and targeted generation of molecular electronic properties such as the HOMO-LUMO gap remain central challenges in computational chemistry and materials discovery. In this work, we investigate three complementary machine learning approaches for HOMO-LUMO gap prediction and conditional molecular generation using the QM9 dataset. First, a descriptor-based LightGBM model is trained with extensive hyperparameter optimization using Optuna and nested cross-validation, achieving a mean absolute error (MAE) of approximately 0.14 eV and demonstrating strong generalization and feature stability. Feature importance analysis highlights the relevance of chemically meaningful descriptors related to electronic and topological structure. Second, a graph convolutional network (GCN) is implemented to directly learn structure-property relationships from molecular graphs. The GCN attains comparable predictive performance with a mean MAE of approximately 0.16 eV and an R$^2$ of 0.97, indicating that graph-based deep learning models can match the accuracy of well-tuned traditional ensemble methods for this task. Finally, a conditional variational autoencoder (cVAE) combining a GCN encoder and a GRU-based SMILES decoder is explored for conditional molecular generation targeting specific HOMO-LUMO gaps. While the cVAE achieves reasonable reconstruction accuracy (mean character-wise reconstruction accuracy of 0.852), the validity and property control of generated molecules remain limited, with large deviations between target and generated HOMO-LUMO gaps. Overall, the results demonstrate that LightGBM and GCN models provide competitive and complementary approaches for HOMO-LUMO gap prediction, while highlighting current challenges in conditional molecular generation and the need for more expressive latent representations and improved training strategies.
\end{abstract}
...
\newline
\noindent
Machine learning provides an efficient alternative by enabling rapid prediction of electronic properties\parencite{Hasan2025}. Crucially, large-scale quantum-chemical datasets like QM9, contain extensive information about molecular structures and properties of organic molecules, enabling the development and benchmarking of ML models with the data volume required for robust learning. Beyond property prediction, ML also enables generative molecular design, allowing in silico exploration and optimization of hypothetical molecules, thereby reducing experimental cost and accelerating molecular discovery.
\newline
\newline
\noindent
In this study, we pursue two objectives. First, we build predictive models for HOMO-LUMO gap estimation using both Light Gradient Boosting Model (LightGBM), a high-performance gradient boosting framework and a graph convolutional network (GCN) that learns molecular representations directly from graph topology. Second, we integrate the GCN into a conditional variational autoencoder (cVAE) to generate novel molecular structures with user-specified target HOMO-LUMO gaps. This combined framework enables both accurate property prediction and inverse molecular design.

\textcolor{purple}{NEED TO ADD A PART ABOUT OUR RESULTS}
