...
The reconstruction accuracy of the cVAE model, using $\overline{CRA}$, across the five outer folds are reported in \cref{tab:CRA}. The model achieved a mean reconstruction accuracy of 0.852, indicating it is capable of reconstructing specific molecules. The significant deviation of fold three and five compared to the others is because of the better set of hyperparameters found in their respective tunings (\cref{tab:cvae_params}). The increased accuracy of folds 3 and 5 originates from the increased learning rate, larger GRU dimension size and the lower $\beta$ parameter.
...
Beyond reconstruction, the more important metric of interest is the generative capability of the cVAE. The chemical validity of the generated molecules, for both the random selected latent vectors and the perturbed latent vectors.

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.5]{img/avg_validity.png}
  \caption{The average validity for both 10 randomly sampled latent vectors and 10 perturbed latent vectors.}
  \label{fig:validity}
\end{figure}
