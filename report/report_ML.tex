...
As both the LightGBM and GCN model predict HOMO-LUMO gaps, and both are eventually compared, the same evaluation metrics are used. Common evaluation metrics in HOMO-LUMO gap prediction are the mean absolute error (MAE), root mean squared error (RMSE), and $R^2$ between the predicted and true HOMO-LUMO gaps. Each metric is calculated as
...
\textcolor{red}{Thibo add the evaluation metric of cVAE}
